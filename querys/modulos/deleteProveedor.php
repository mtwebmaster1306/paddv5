<?php
error_log("deleteProveedor.php se ha ejecutado");
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $url = SUPABASE_URL . "/rest/v1/Proveedores?id_proveedor=eq." . urlencode($id);
    ...
    if ($resultado === true) {
        error_log("Proveedor eliminado con éxito");
        echo json_encode(['success' => true, 'message' => 'Proveedor eliminado con éxito']);
    } else {
        error_log("Error al eliminar el Proveedor. Detalles: " . json_encode($resultado));
        echo json_encode(['success' => false, 'message' => 'Error al eliminar el Proveedor', 'details' => $resultado]);
    }
} else {
